...
const faces = (
  await computerVisionClient.analyzeImage(facesImageURL, {
...
if (objects.length) {
  ...
  for (const obj of objects) {
    console.log(
      `    ${obj.object} (${obj.confidence.toFixed(2)}) at ${formatRectObjects(
        obj.rectangle
      )}`
    );
  }
...
const categories = (await computerVisionClient.analyzeImage(categoryURLImage))
  .categories;
...
const domain = (
  await computerVisionClient.analyzeImageByDomain("landmarks", domainURLImage)
).result.landmarks;

// Prints domain-specific, recognized objects
if (domain.length) {
  console.log(
    `${domain.length} ${domain.length == 1 ? "landmark" : "landmarks"} found:`
  );
